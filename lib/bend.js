...
TokenKind.LPARAM = new TokenKind("LPARAM", null);
TokenKind.RPARAM = new TokenKind("RPARAM", null);
...
Tokenizer.tokenize = function(log, file, text) {
  ...
  while (i < tokens.length) {
    ...
    // Change parantheses to tokens for function types
    if (token2.kind === TokenKind.LPAREN && i >= 2 && tokens[i - 2].kind === TokenKind.FN) {
      token2.kind = TokenKind.LARGS;
    } else if (token2.kind === TokenKind.RPAREN && stack.length > 0 && stack[stack.length - 1].kind === TokenKind.LARGS) {
      token2.kind = TokenKind.RARGS;
    }

    // Break out of all type parameter lists for non-special tokens
    if (TokenKind.TYPE_PARAMETER_LIST.indexOf(token2.kind) < 0) {
      while (stack.length > 0 && stack[stack.length - 1].kind === TokenKind.LT) {
        stack.pop();
      }
    }
    ...
    if (TokenKind.LEFT_BRACKETS.indexOf(token2.kind) >= 0) {
      ...
      // Convert parentheses to argument tokens for lambdas
      ...
      // Convert angle brackets to type parameter tokens, splitting compound tokens
      if (top.kind === TokenKind.LT) {
        if (token2.kind === TokenKind.SHR) {
          tokens.splice(i, 0, new Token(token2.location, TokenKind.GT, ">"));
          token2.text = ">";
        } else if (token2.kind === TokenKind.GTE) {
          tokens.splice(i, 0, new Token(token2.location, TokenKind.ASSIGN, "="));
          token2.text = ">";
        }
        top.kind = TokenKind.LPARAM;
        token2.kind = TokenKind.RPARAM;
      }
    }
  }
  return tokens;
...
// class ClassDef
var ClassDef = function(location, comment, id, modifiers, baseType, body, params) {
  Def.call(this, location, comment, id, modifiers);
...
// class FuncTypeExpr
var FuncTypeExpr = function(location, returnType, argTypes) {
  Expr.call(this, location);
  this.returnType = returnType;
...
// class SpecificTypeExpr
var SpecificTypeExpr = function(location, classType, paramTypes) {
  Expr.call(this, location);
  this.classType = classType;
  this.paramTypes = paramTypes;
};
SpecificTypeExpr.prototype = Object.create(Expr.prototype);
SpecificTypeExpr.prototype.accept = function(v) {
  v.visitSpecificTypeExpr(this);
};
...
Visitor.prototype.visitFuncTypeExpr = function(node) {
  this.visit(node.returnType);
  for (var i = 0; i < node.argTypes.length; i++) {
    var argType = node.argTypes[i];
    this.visit(argType);
  }
};
...
Visitor.prototype.visitSpecificTypeExpr = function(node) {
  this.visit(node.classType);
  for (var i = 0; i < node.paramTypes.length; i++) {
    var paramType = node.paramTypes[i];
...
SymbolKind.PARAM = new SymbolKind();
...
Type.isArray = function(type) {
  var type2;
  return (type2 = type) instanceof SpecificType && type2.classType === ClassType.ARRAY;
};
...
// 
Type.isGeneric = function(type) {
  var type2, type3, type4, type5, type6;
  if ((type2 = type) instanceof MetaType) {
    return Type.isGeneric(type2.instanceType);
  }
  if ((type3 = type) instanceof NullableType) {
    return Type.isGeneric(type3.innerType);
  }
  if ((type4 = type) instanceof FuncType) {
    if (Type.isGeneric(type4.returnType)) {
      return true;
    }
    for (var i = 0; i < type4.argTypes.length; i++) {
      var argType = type4.argTypes[i];
  ...
  if ((type5 = type) instanceof ClassType) {
    return type5.params.length > 0;
  }
  if ((type6 = type) instanceof SpecificType) {
    for (var i2 = 0; i2 < type6.paramTypes.length; i2++) {
      var paramType = type6.paramTypes[i2];
      if (Type.isGeneric(paramType)) {
        return true;
      }
    }
    return false;
...
Type.removeParams = function(type, params) {
  ...
  if ((type2 = type) instanceof TypeParam) {
    if (type2.index >= params.length) {
      throw new Error("fail in src/type.bend on line 36");
    }
  ...
  if ((type4 = type) instanceof FuncType) {
    var returnType = Type.removeParams(type4.returnType, params);
    var changed = returnType !== type4.returnType;
    var argTypes = [];
    for (var i = 0; i < type4.argTypes.length; i++) {
      var oldArgType = type4.argTypes[i];
    ...
    var funcType = new FuncType(returnType, argTypes, type4.minArgCount);
    funcType.splatIndex = type4.splatIndex;
    funcType.argNames = type4.argNames;
    return funcType;
  }
  if ((type5 = type) instanceof ClassType && type5.params.length > 0) {
    var paramTypes = [];
    for (var i2 = 0; i2 < type5.params.length; i2++) {
      var oldParamType = type5.params[i2];
      var newParamType = Type.removeParams(oldParamType, params);
      if (newParamType instanceof TypeParam) {
        return type5;
      }
      paramTypes.push(newParamType);
    }
    return new SpecificType(type5, paramTypes);
  }
  if ((type6 = type) instanceof SpecificType) {
    var changed2 = false;
    var paramTypes2 = [];
    for (var i3 = 0; i3 < type6.paramTypes.length; i3++) {
      var oldParamType2 = type6.paramTypes[i3];
      var newParamType2 = Type.removeParams(oldParamType2, params);
      paramTypes2.push(newParamType2);
      if (newParamType2 !== oldParamType2) {
        changed2 = true;
      }
    }
    if (!changed2) {
    ...
    return new SpecificType(type6.classType, paramTypes2);
  }
  return type;
};
...
Type.equals = function(a, b) {
  ...
  if ((a4 = a) instanceof FuncType && (b4 = b) instanceof FuncType) {
    if (!Type.equals(a4.returnType, b4.returnType) || a4.argTypes.length !== b4.argTypes.length || a4.minArgCount !== b4.minArgCount) {
      return false;
    }
    var i = 0;
    while (i < a4.argTypes.length) {
      if (!Type.equals(a4.argTypes[i], b4.argTypes[i])) {
        return false;
      }
      i++;
    }
  ...
  if ((a5 = a) instanceof SpecificType && (b5 = b) instanceof SpecificType) {
    if (!Type.equals(a5.classType, b5.classType) || a5.paramTypes.length !== b5.paramTypes.length) {
      return false;
    }
    var i2 = 0;
    while (i2 < a5.paramTypes.length) {
      if (!Type.equals(a5.paramTypes[i2], b5.paramTypes[i2])) {
        return false;
      }
      i2++;
    }
...
// class TypeParam
var TypeParam = function(name, index) {
  Type.call(this);
  this.name = name;
  this.index = index;
...
// class ClassType
var ClassType = function(name, staticScope, instanceScope, isAbstract, params) {
  Type.call(this);
  this.name = name;
...
ClassType.makeNative = function(name, isArray) {
  ...
  if (isArray) {
    ...
    classType.params.push(new TypeParam("T", 0));
  }
  return classType;
};
ClassType.INT = ClassType.makeNative("int", false);
ClassType.BOOL = ClassType.makeNative("bool", false);
ClassType.FLOAT = ClassType.makeNative("float", false);
ClassType.STRING = ClassType.makeNative("string", false);
ClassType.ARRAY = ClassType.makeNative("array", true);

// class SpecificType
var SpecificType = function(classType, paramTypes) {
  Type.call(this);
  this.classType = classType;
  this.paramTypes = paramTypes;
};
SpecificType.prototype = Object.create(Type.prototype);
SpecificType.prototype.toString = function() {
  if (this.classType === ClassType.ARRAY && this.paramTypes.length === 1) {
    return this.paramTypes[0] + "[]";
  }
  return this.classType.name + "<" + this.paramTypes.join(", ") + ">";
};
...
var Parser = function() {
  ...
  this.pratt.get(TokenKind.FN, TIGHT_PRECEDENCE).infixParselet = function(context, left) {
    ...
    var location = context.current().location;
    context.next();
    if (!context.expect(TokenKind.LARGS)) {
      return null;
    }
    var args = [];
  ...
  // Resolved class type
  this.pratt.get(TokenKind.LPARAM, TIGHT_PRECEDENCE).infixParselet = function(context, left) {
    var param;
    var location = context.current().location;
    context.next();
    var params = [];
    while (!context.eat(TokenKind.RPARAM)) {
      if (params.length > 0 && !context.expect(TokenKind.COMMA)) {
        return null;
      }
      if ((param = that.pratt.parse(context, 0)) !== null) {
        params.push(param);
      } else {
        return null;
      }
    }
    return new SpecificTypeExpr(location, left, params);
  };
    ...
    if (context.eat(TokenKind.CLASS)) {
      ...
      if (context.eat(TokenKind.LPARAM)) {
        while (!context.eat(TokenKind.RPARAM)) {
          ...
          params.push(new TypeParam(context.current().text, params.length));
          if (!context.expect(TokenKind.IDENT)) {
            return null;
          }
        }
      ...
      if ((body4 = this.parseBlock(context, BlockKind.CLASS, id2.name)) !== null) {
        var classDef = new ClassDef(location, comment, id2, modifiers, baseType, body4, params);
...
CreateScopesPass.prototype.visitClassDef = function(node) {
  ...
  var instanceScope = new Scope(ScopeKind.INSTANCE, staticScope, null);
  var classType = new ClassType(node.id.name, staticScope, instanceScope, (node.modifiers & Modifier.ABSTRACT) !== 0, node.params);
  ...
  for (var i = 0; i < node.params.length; i++) {
    ...
    staticScope.define(this.log, node.location, SymbolKind.PARAM, param.name, param.metaType, Modifier.STATIC);
  }
  for (var i2 = 0; i2 < node.body.stmts.length; i2++) {
    var stmt = node.body.stmts[i2];
...
CreateScopesPass.prototype.visitSpecificTypeExpr = function(node) {
  node.scope = this.scope;
  Pass.prototype.visitSpecificTypeExpr.call(this, node);
};
...
ResolveSymbolTypesPass.prototype.checkInstantiable = function(location, type) {
  if (Type.isGeneric(type)) {
    this.log.error(location, "cannot instantiate generic type " + type);
  }
};
ResolveSymbolTypesPass.prototype.visitVarDef = function(node) {
  ...
  if ((symbol = node.id.symbol) !== null) {
    ...
    this.checkInstantiable(node.location, symbol.type);
  }
};
...
// class CtorCallInfo
var CtorCallInfo = function(ctorSymbol, resolvedType) {
  this.ctorSymbol = ctorSymbol;
  ...
  if (args.length < funcType.minArgCount) {
    ...
    while (i < args.length) {
      ...
      if (funcType.splatIndex !== -1 && i >= funcType.splatIndex) {
        ...
        } else if ((type = funcType.argTypes[funcType.splatIndex]) instanceof SpecificType && type.classType === ClassType.ARRAY) {
...
ResolveTypesPass.prototype.visitForStmt = function(node) {
  ...
  this.visit(node.id);
  if ((symbol = node.id.symbol) !== null) {
    if ((type = node.value.resolvedType) instanceof SpecificType && type.classType === ClassType.ARRAY) {
...
ResolveTypesPass.prototype.visitBinaryExpr = function(node) {
  ...
  if (node.op === BinaryOp.ASSIGN) {
    ...
    //   if x in { 1, 2, 3 } { ... }
    // 
    this.visitWithContext(new SpecificType(ClassType.ARRAY, [node.left.resolvedType]), node.right);
  } else {
    this.visit(node.right);
  }
};
...
ResolveTypesPass.prototype.callCtorInfo = function(location, args, type) {
  var that = this, type2, classType;

  // Common logic for class constructors
  var handleClassType = function(classType, patch) {
    var ctorSymbol, ctorType;
    if (classType.isAbstract) {
      that.log.error(location, "cannot instantiate abstract class " + classType);
      return null;
    } else if ((ctorSymbol = classType.instanceScope.lookup("this", true)) !== null && (ctorType = patch(ctorSymbol.type)) instanceof FuncType) {
      that.visitCallArgs(location, ctorType, args);
      return new CtorCallInfo(ctorSymbol, type);
    }
    that.log.error(location, type + " has no constructor");
    return null;
  };
  if ((type2 = type) instanceof SpecificType) {
    // Resolve generic constructors before calling them
    return handleClassType(type2.classType, function(ctorType) {
      return Type.removeParams(ctorType, type2.paramTypes);
    });
  } else if ((classType = type) instanceof ClassType) {
    // Normal class constructor
    return handleClassType(classType, function(ctorType) {
      return ctorType;
    });
  } else if (type instanceof FuncType || type === SpecialType.VAR) {
    // Special-case constructors for functions and var
    this.visitCallArgs(location, new FuncType(type, [type], 1), args);
    return new CtorCallInfo(null, type);
  }
  this.log.error(location, type + " has no constructor");
  return null;
};
ResolveTypesPass.prototype.visitCallExpr = function(node) {
  var funcType, metaType, info, classType, ctorSymbol, funcType2;
  this.visit(node.value);
  if ((funcType = node.value.resolvedType) instanceof FuncType) {
    ...
    if ((info = this.callCtorInfo(node.location, node.args, metaType.instanceType)) !== null) {
      node.ctorSymbol = info.ctorSymbol;
      node.resolvedType = info.resolvedType;
    }
  } else if ((node.value instanceof ThisExpr || node.value instanceof BaseExpr) && (classType = node.value.resolvedType) instanceof ClassType) {
    ...
    if ((ctorSymbol = node.ctorSymbol) !== null && (funcType2 = ctorSymbol.type) instanceof FuncType) {
      ...
      throw new Error("fail in src/pass.bend on line 756");
    }
  } else if (node.value.resolvedType === SpecialType.VAR) {
    for (var i = 0; i < node.args.length; i++) {
      var arg = node.args[i];
      this.visitWithContext(SpecialType.VAR, arg);
    }
...
ResolveTypesPass.prototype.visitMemberExpr = function(node) {
  var metaType, classType, instantiatedType, classType2, instantiatedType2, scope, symbol;
  this.visit(node.value);
  ...
  if ((metaType = node.value.resolvedType) instanceof MetaType) {
    ...
    } else if ((instantiatedType = metaType.instanceType) instanceof SpecificType) {
  ...
  } else if ((instantiatedType2 = node.value.resolvedType) instanceof SpecificType) {
    maybeScope = instantiatedType2.classType.instanceScope;
    paramTypes = instantiatedType2.paramTypes;
  } else if (node.value.resolvedType === SpecialType.VAR) {
    node.resolvedType = SpecialType.VAR;
    return;
  }
  ...
  if ((scope = maybeScope) !== null) {
    ...
    if ((symbol = scope.lookup(node.id.name, true)) !== null) {
      ...
      if (paramTypes.length > 0) {
        node.resolvedType = Type.removeParams(node.resolvedType, paramTypes);
      }
    } else {
...
ResolveTypesPass.prototype.visitInitExpr = function(node) {
  var context, info;
  if ((context = this.context) !== null && context.node === node) {
    if ((info = this.callCtorInfo(node.location, node.values, context.targetType)) !== null) {
      node.ctorSymbol = info.ctorSymbol;
      node.resolvedType = info.resolvedType;
    }
  } else {
...
ResolveTypesPass.prototype.visitMatchExpr = function(node) {
  ...
  if ((symbol = node.id.symbol) !== null) {
    ...
    } else if (symbol.type instanceof NullableType) {
...
ResolveTypesPass.prototype.visitArrayTypeExpr = function(node) {
  ...
  if (elementType !== SpecialType.ERROR) {
    node.resolvedType = new MetaType(new SpecificType(ClassType.ARRAY, [elementType]));
  }
};
ResolveTypesPass.prototype.visitFuncTypeExpr = function(node) {
  ...
  for (var i = 0; i < node.argTypes.length; i++) {
    var argType = node.argTypes[i];
    var type = this.instanceType(argType, false);
    if (type === SpecialType.ERROR) {
      return;
    }
    argTypes.push(type);
  }
...
ResolveTypesPass.prototype.visitSpecificTypeExpr = function(node) {
  var classType2;
  Pass.prototype.visitSpecificTypeExpr.call(this, node);
  var classType = this.instanceType(node.classType, false);
  if ((classType2 = classType) instanceof ClassType && classType2.params.length > 0) {
    if (classType2.isAbstract) {
      this.log.error(node.location, "cannot instantiate abstract class " + classType2);
    } else if (classType2.params.length === node.paramTypes.length) {
      var paramTypes = [];
      for (var i = 0; i < node.paramTypes.length; i++) {
        var param = node.paramTypes[i];
        var type = this.instanceType(param, false);
        if (type === SpecialType.ERROR) {
          return;
        }
        paramTypes.push(type);
      }
      node.resolvedType = new MetaType(new SpecificType(classType2, paramTypes));
    } else {
      this.log.error(node.location, "expected " + classType2.params.length + " type parameters but got " + node.paramTypes.length);
    }
  } else {
    this.log.error(node.location, classType + " has no free type parameters");
  }
};
...
SpecialScope.init = function() {
  ...
  var ARRAY = ClassType.ARRAY;
  var T = ARRAY.params[0];
  var VAR = SpecialType.VAR;
  var VOID = SpecialType.VOID;
  var ARRAY_T = new SpecificType(ARRAY, [T]);
  var ARRAY_VAR = new SpecificType(ARRAY, [VAR]);
  var ARRAY_STRING = new SpecificType(ARRAY, [STRING]);
  ...
  SpecialScope.func(STRING.instanceScope, "split", new FuncType(ARRAY_STRING, [STRING], 1));
  ...
  SpecialScope.field(ARRAY.instanceScope, "size", INT);
  SpecialScope.func(ARRAY.instanceScope, "push", new FuncType(VOID, [T], 1));
  SpecialScope.func(ARRAY.instanceScope, "pop", new FuncType(T, [], 0));
  SpecialScope.func(ARRAY.instanceScope, "unshift", new FuncType(VOID, [T], 1));
  SpecialScope.func(ARRAY.instanceScope, "shift", new FuncType(T, [], 0));
  SpecialScope.func(ARRAY.instanceScope, "splice", new FuncType(ARRAY_T, [INT, INT, ARRAY_T], 2));
  SpecialScope.func(ARRAY.instanceScope, "join", new FuncType(STRING, [STRING], 1));
  SpecialScope.func(ARRAY.instanceScope, "filter", new FuncType(ARRAY_T, [new FuncType(BOOL, [T], 1)], 1));
  SpecialScope.func(ARRAY.instanceScope, "slice", new FuncType(ARRAY_T, [INT, INT], 0));
  SpecialScope.func(ARRAY.instanceScope, "map", new FuncType(ARRAY_VAR, [new FuncType(VAR, [T], 1)], 1));
  SpecialScope.func(ARRAY.instanceScope, "concat", new FuncType(ARRAY_T, [ARRAY_T], 1));
  SpecialScope.func(ARRAY.instanceScope, "indexOf", new FuncType(INT, [T], 1));
  SpecialScope.func(ARRAY.instanceScope, "reverse", new FuncType(ARRAY_T, [], 0));
  if ((symbol = ARRAY.instanceScope.find("splice")) !== null && (funcType = symbol.type) instanceof FuncType) {
...
// class Handler
JsNative.Handler = function(symbol, callback) {
  this.symbol = symbol;
  this.callback = callback;
};
JsNative.handlers = [];
JsNative.register = function(symbol, callback) {
  JsNative.handlers.push(new JsNative.Handler(symbol, callback));
};
JsNative.handleCall = function(symbol, target, args, visit) {
  for (var i = 0; i < JsNative.handlers.length; i++) {
    var handler = JsNative.handlers[i];
...
JsNative.get = function(scope, name) {
  ...
  throw new Error("fail in src/native.bend on line 94");
};
  ...
  while (i < text.length) {
    ...
    if (code === 9) {
      ...
      if (word.length === 1) {
        ...
        throw new Error("fail in src/native.bend on line 119");
      }
...
JsNative.init = function() {
  JsNative.register(JsNative.get(ClassType.INT.instanceScope, "this"), function(visit, target, args) {
    return new JsBinary(visit(args[0]), JsBinaryOp.BITOR, new JsText("0"));
  });
  JsNative.register(JsNative.get(ClassType.BOOL.instanceScope, "this"), function(visit, target, args) {
    return visit(args[0]);
  });
  JsNative.register(JsNative.get(ClassType.FLOAT.instanceScope, "this"), function(visit, target, args) {
    return visit(args[0]);
  });
  JsNative.register(JsNative.get(ClassType.STRING.instanceScope, "this"), function(visit, target, args) {
    return visit(args[0]);
  });
  JsNative.register(JsNative.get(ClassType.ARRAY.instanceScope, "this"), function(visit, target, args) {
    return new JsArray(args.map(visit));
  });
  JsNative.register(JsNative.get(ClassType.INT.instanceScope, "toHex"), function(visit, target, args) {
    return new JsCall(new JsBinary(visit(target), JsBinaryOp.MEMBER, new JsText("toString")), [new JsText("16")]);
  });
  JsNative.register(JsNative.get(ClassType.STRING.instanceScope, "quote"), function(visit, target, args) {
    return new JsCall(new JsBinary(new JsText("JSON"), JsBinaryOp.MEMBER, new JsText("stringify")), [visit(target)]);
  });
  JsNative.register(JsNative.get(ClassType.STRING.instanceScope, "toInt"), function(visit, target, args) {
    return new JsBinary(visit(target), JsBinaryOp.BITOR, new JsText("0"));
  });
  JsNative.register(JsNative.get(ClassType.STRING.instanceScope, "toFloat"), function(visit, target, args) {
    return new JsUnary(visit(target), JsUnaryOp.POS);
  });
  JsNative.register(JsNative.get(ClassType.STRING.instanceScope, "replace"), function(visit, target, args) {
    var arg0;
    var ESCAPE_REGEX = "([\\.\\?\\*\\+\\^\\$\\[\\]\\/\\\\\\(\\)\\{\\}\\|\\-])";
    if ((arg0 = args[0]) instanceof StringExpr) {
      return new JsCall(new JsBinary(visit(target), JsBinaryOp.MEMBER, new JsText("replace")), [
        new JsText("/" + JsNative.escapeNonASCII(arg0.value.replace(new RegExp(ESCAPE_REGEX, "g"), "\\$1")) + "/g"),
        visit(args[1])
      ]);
    }
    return new JsCall(new JsBinary(visit(target), JsBinaryOp.MEMBER, new JsText("replace")), [
      new JsCall(new JsUnary(new JsText("RegExp"), JsUnaryOp.NEW), [
        new JsCall(new JsBinary(visit(args[0]), JsBinaryOp.MEMBER, new JsText("replace")), [
  ...
  JsNative.register(JsNative.get(ClassType.STRING.instanceScope, "replacePattern"), function(visit, target, args) {
    var arg0;
    if ((arg0 = args[0]) instanceof StringExpr) {
      return new JsCall(new JsBinary(visit(target), JsBinaryOp.MEMBER, new JsText("replace")), [
        new JsText("/" + JsNative.escapeNonASCII(arg0.value.replace(/\//g, "\\/")) + "/g"),
        visit(args[1])
      ]);
    }
    return new JsCall(new JsBinary(visit(target), JsBinaryOp.MEMBER, new JsText("replace")), [
  ...
  JsNative.register(JsNative.get(ClassType.STRING.staticScope, "fromCharCode"), function(visit, target, args) {
    return new JsCall(new JsBinary(new JsText("String"), JsBinaryOp.MEMBER, new JsText("fromCharCode")), [visit(args[0])]);
  });
};
SpecialScope.init();
JsNative.init();
  ...
  if ((node2 = node) instanceof Module) {
    ...
    if ((value2 = node8.value) instanceof MemberExpr && (symbol2 = value2.id.symbol) !== null && (baseValue = value2.value) instanceof BaseExpr && (baseType = baseValue.resolvedType) instanceof ClassType) {
      ...
      if ((metaType2 = node8.value.resolvedType) instanceof MetaType) {
        if ((metaType2.instanceType instanceof FuncType || metaType2.instanceType === SpecialType.VAR) && args4.length === 1) {
          return args4[0];
        } else {
          throw new Error("fail in src/js.bend on line 561");
        }
      }
    ...
    if ((node10.value.resolvedType === ClassType.STRING || node10.value.resolvedType instanceof SpecificType) && (symbol4 = node10.id.symbol) !== null && symbol4.name === "size") {
      ...
      throw new Error("fail in src/js.bend on line 580");
    }
    ...
    if ((symbol7 = node16.symbol) !== null) {
      ...
      throw new Error("fail in src/js.bend on line 639");
    }
    ...
    throw new Error("fail in src/js.bend on line 654");
  }
};
